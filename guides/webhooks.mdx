...
Choose which of the six event types (below) your endpoint should receive. Subscribe only to what you act on; you can change subscriptions at any time.
...
| `bridge.completed` | A [bridge](/api-reference/bridge) has settled: cNGN was minted on the destination network. |
...
<Tab title="bridge.completed">

Fires when a [bridge](/api-reference/bridge) settles, meaning the cNGN has been minted on the destination network. `network` is the destination network, `base_trx_hash` is the burn on the origin network, `extl_trx_hash` is the mint on the destination network, `receiver` is the destination address, and `amount` is the amount bridged before fees:
...
A bridge that fails fires `transaction.failed` with `trx_type` `swap` instead.
